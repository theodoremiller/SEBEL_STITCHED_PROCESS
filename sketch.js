...
const appendImageArr = name => {
  imageImportArray.push(name);
  checker[imageImportArray.length-1] = true;
...
function preload(){
  nearInfared = loadImage(compositeImageFilename, () => appendImageArr("Composite Image"), () => console.error(`${compositeImageFilename} not found, but is necessary.  Stopping,`));
  rgb = loadImage(rgbImageFilename, () => appendImageArr("RGB Image"), () => console.error(`${compositeImageFilename} not found, but is necessary.  Stopping.`));
  netRadiationImageRn = loadImage(netRadiationImageFilename, () => appendImageArr("Net Radiation Image"), () => console.log(`${netRadiationImageFilename} not found, skipping.`));
  soilFluxImageG = loadImage(soilFluxImageFilename, () => appendImageArr("Soil Flux Image"), () => console.log(`${soilFluxImageFilename} not found, skipping.`));
  thermalImageImp = loadImage(thermalImageFilename, () => appendImageArr("Thermal Image"), () => console.log(`${thermalImageFilename} not found, skipping.`));
  sensibleHeatFluxImage = loadImage(sensHeatFluxFilename, () => appendImageArr("Sensible Heat Flux"), () => console.log(`${sensHeatFluxFilename} not found, skipping.`));
}
...
function setup() {
  ...
  tf.tidy(() => {
    ...
    if(!checkRequirements([0,1])) console.error("Must have at least an RGB image, and a composite image where the Red channel contains the red values, the green channel contains the thermal bands, and the blue channel contains the near-infared band.");
  ...
  // --- Inputs ---
    ...
    thermalTensor = tf.tensor(RawThermal,[imgW,imgH]);
    RawThermal = null;
  // --------------
  ...
  // --- Outputs ---
    let RawSensibleHeatFlux = mapTensor(sensibleHeatFluxH, --- , --- ,0,255).toInt().dataSync();
